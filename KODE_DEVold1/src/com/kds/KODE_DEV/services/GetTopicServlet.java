...
public class GetTopicServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			String sql = "select topic_id, topicname from topic_description where subject_id=?";
			...
			rs = ps.executeQuery();
			List<TopicInfoDomain> topics = new  ArrayList<TopicInfoDomain>();
			System.out.println("Query :"+sql);
			while (rs.next()) {
				TopicInfoDomain topic = new TopicInfoDomain();
				System.out.println("Elements :" + rs.getString(1));
				topic.setTopicId(rs.getString(1));
				topic.setTopic(rs.getString(2));
				topics.add(topic);
			}
			json = new Gson().toJson(topics);
