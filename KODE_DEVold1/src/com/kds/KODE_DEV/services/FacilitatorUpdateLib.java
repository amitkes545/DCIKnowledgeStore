...
public class FacilitatorUpdateLib extends CommonService {

	static final Logger LOGGER = Logger.getLogger(FacilitatorUpdateLib.class);
	...
	public void run() throws ServletException, IOException {
		...
		int libsizechange=0;  //By Prity
		long libsizechangeLong=0l;
		for(int i=0; i<checkboxGroup.length; i++){
			 fmksDomain = new FacilitatorManageKnowStoreDomain();
			fmksDomain.setLibName(request.getParameter("libName"+checkboxGroup[i]).trim());
		...
		//	fmksDomain.setStudentId(request.getParameter("oldlibName"+checkboxGroup[i]));
			fmksDomain.setStudentId(request.getParameter("libName"+checkboxGroup[i]).trim());
			String spaceUom=request.getParameter("spaceuom"+checkboxGroup[i]);
			String oldSpaceUom=request.getParameter("oldSpaceUom"+checkboxGroup[i]);
			System.out.println("Space UOM : "+spaceUom);
			fmksDomain.setSpaceUom(spaceUom);
			arl.add(fmksDomain);
			
			String new_lib=request.getParameter("libName"+checkboxGroup[i]).trim();
			String old_lib=request.getParameter("oldlibName"+checkboxGroup[i]).trim();
			
			//By Prity
			int old_libsize=Integer.parseInt(request.getParameter("oldlibSize"+checkboxGroup[i]));
			libsizechange = libsizechange + (libSize1 - old_libsize);
			long libSize1Long=Long.parseLong(Utilities.getBytes(libSize1, spaceUom));
			long old_libsizeLong=Long.parseLong(Utilities.getBytes(old_libsize, oldSpaceUom));
			libsizechangeLong=libsizechangeLong+(libSize1Long - old_libsizeLong);
			if(libSize1Long<old_libsizeLong)
			{
				msg = "Lib size smaller  !!";  //By Prity
		...
		//}
			if (isDuplicate) {
				msg = "Lib name already exists !!";  //By Prity
				System.out.println("lib name already exists");
				arl.clear();
				break;
			}
		...
		//if(remainingSize < libsizechange){
		if(remainingSizeLong < libsizechangeLong){
			msg = "You don't have enough Space !!";
		...
		if(arl.size()>0 && remainingSizeLong >= libsizechangeLong ){  //condition change by Prity
			System.out.println("in arl .size");
/*		Commented by Prity as renaming of folder in ftp is not needed as it is named with lib id and not with lib name	
...
*/		
		...
		///if(allLibSizeOfDbInt>=libSize1){ Commented by Prity
		//if(allLibSizeOfDbInt>=libSize1){
			...
			if (s1.equals("success")) {
				 msg = "Library modified successfully";
