...
public class DeleteAssignmentsServlet extends CommonService {

	static final Logger LOGGER = Logger.getLogger(DeleteAssignmentsServlet.class);
...
System.out.println("in service");
		...
		String assesmentId = request.getParameter("assessid");
		...
		recipientTypeForAssessment = searchDao.getRecipientType(assessmentDomain,orgIdInSession,facultyIdInSession);
		}
		else if(assesmentId.equalsIgnoreCase("All")){
			SearchDao searchDao = new SearchDao();

			// This method for getting all Recipient Type From assessment_info table
		recipientTypeForAssessment = searchDao.getAllRecipientType(assessmentDomain, orgIdInSession,facultyIdInSession);	
		}
		// setting values for getting in jsp
		if (recipientTypeForAssessment.size() != 0) {
			request.setAttribute("recipientTypeForAssessment",recipientTypeForAssessment);
			request.setAttribute("asessID", assesmentId);
			String Tabmsg = "tab1";
			RequestDispatcher requestDispatcher = request
					.getRequestDispatcher("../JSP/DeleteAssignments1.jsp?tabValue="+Tabmsg);
