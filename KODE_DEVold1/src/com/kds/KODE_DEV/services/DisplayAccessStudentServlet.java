...
public class DisplayAccessStudentServlet extends CommonService{
	
	static final Logger LOGGER = Logger.getLogger(DisplayAccessStudentServlet.class);
	public void run() throws ServletException,IOException {
		
		session=request.getSession();
		AdminDomain adomain=new AdminDomain();
		AdminAccessControlDao adminAccessControlDao=new AdminAccessControlDao();
		...
		String studentId=request.getParameter("selectedUserId");
		String status=null;
    	LOGGER.info("userid is"+userid+ "studentId:"+studentId);
     ...
     if(listFromDb.size()>0 && resultList.size()==0){
		...
		request.setAttribute("AdminSuccess",OrgStatus);
		...
		requestDispatcher.forward(request, response);
    	 ...
    	 String OrgStatus="values are not updated";
    	 request.setAttribute("AdminSuccess",OrgStatus);
    	 RequestDispatcher requestDispatcher=request.getRequestDispatcher("../JSP/DisplayAccessStudent.jsp");
 		requestDispatcher.forward(request, response);
     }
   ...
   // session.removeAttribute("AvailableValueinDB");
    ...
    if(listFromDb.size()==0 && resultList.size()>0){
    	LOGGER.info("listfrom db:"+listFromDb);
          status= adminAccessControlDao.insertAdminAccessDao(studentId, resultList);
          ...
          if("success".equalsIgnoreCase(status)){
 			...
 			// LOGGER.info("process id are"+AvailableValueinDB);
  			...
  			request.setAttribute("AdminFailure",OrgStatus);
  			RequestDispatcher requestDispatcher=request.getRequestDispatcher("../JSP/DisplayAccessStudent.jsp");
  			requestDispatcher.forward(request, response);
          }
    }
    	...
    	if(finalListToAdd.size()>0) {
    		LOGGER.info("finalListToAdd"+finalListToAdd);
    		 status= adminAccessControlDao.updateAdminAccessDao(studentId, finalListToAdd);
    	...
    	if(finalListToDelete.size()>0) {
    		...
    		status=adminAccessControlDao.deleteAdminAccessDao(studentId, finalListToDelete);
