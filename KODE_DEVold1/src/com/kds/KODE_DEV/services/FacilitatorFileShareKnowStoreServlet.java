...
public class FacilitatorFileShareKnowStoreServlet extends CommonService {
	
	static final Logger LOGGER = Logger.getLogger(FacilitatorFileShareKnowStoreServlet.class);
	...
	public void run() throws Exception {
		...
		if (isMultiPart) 
		{
			...
			LOGGER.info("recipientType "+recipientType);
			//recipientType=recipientType.substring(beginIndex, endIndex)length()
			/*
			...
			// Object[] s1=items.get(4).toString();
			 LOGGER.info("ksID "+ksID.getString());
			 LOGGER.info("departMent "+departMent.getString());
			 LOGGER.info("subject "+subject.getString());
			 LOGGER.info("description "+description.getString());
			//   LOGGER.info("recipientType"+recipientType.length);
			// LOGGER.info();
	...
	//		String password = "ftp@123#1";
			...
			try {
				...
				if (result == true) {
					...
					//boolean folderCreated = ftpclient.makeDirectory(directoryPath);
					...
					if (true) {
						...
						LOGGER.info(file);
						 testName = fileName;
						 directoryPath="/home/ftpkds1/KnowStore/"+session.getAttribute("orgid")+ '/'+ session.getAttribute("userid");
						...
						response.sendRedirect("../JSP/FacilitatorFileShareKnowStore.jsp");
					}
					}
				} else {
