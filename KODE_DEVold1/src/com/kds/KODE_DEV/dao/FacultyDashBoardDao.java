	...
	ResultSet resultSet = null;
	 ...
	 public ArrayList<AssessmentDomain> selectAsssesmentName(UsersInfoDomain uid){
		...
		//	DBTransaction1 dbt1 = new DBTransaction1();
			...
			try {
				...
				String quary = "SELECT assessment_name,uploaded_time,uploaded_date FROM assessment_info where organization_id='"+uid.getOrgId()+"' and uploaded_by='"+uid.getUserId()+"' order by uploaded_date desc,uploaded_time desc limit 9";
				//System.out.println(quary);
	 ...
	 public ArrayList<AssessmentDomain> selectAsssignmentName(UsersInfoDomain uid){
		...
		//	DBTransaction1 dbt1 = new DBTransaction1();
			...
			try {
				...
				String quary = "SELECT assignment_name,uploaded_time,uploaded_date FROM assignment_info where organization_id='"+uid.getOrgId()+"' and uploaded_by='"+uid.getUserId()+"' limit 5";
				//System.out.println(quary);
				prepareStatement = connection.prepareStatement(quary);
				resultSet = prepareStatement.executeQuery();
				while(resultSet.next())
	 ...
	 public ArrayList<SessionDomain> selectSessionDetails(UsersInfoDomain uid){
		...
		//	DBTransaction1 dbt1 = new DBTransaction1();
			...
			//	String quary = "SELECT session_name,session_start_time,session_end_time,session_id FROM session_details where organization_id='"+uid.getOrgId()+"' and faculty_name='"+uid.getUserId()+"' order by session_start_time desc limit 9";
				String quary = "SELECT session_name,session_start_time,session_end_time,session_id FROM session_details where organization_id='"+uid.getOrgId()+"' and faculty_name='"+uid.getUserId()+"' and session_status='Available' and cast(session_end_time as timestamp) > now() order by session_start_time limit 10";
				//System.out.println(quary);
				prepareStatement = connection.prepareStatement(quary);
				resultSet = prepareStatement.executeQuery();
				while(resultSet.next())
		...
		//	System.out.println("session_end_time name::"+resultSet.getString("session_end_time"));
					//domain.set
					//dcd.setCourseImageLocation(rs.getString("uploaded_by"));
					arl.add(domain);
				}
