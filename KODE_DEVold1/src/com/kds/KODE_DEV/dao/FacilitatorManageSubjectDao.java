...
public class FacilitatorManageSubjectDao {
	Connection con = null;
	...
	public ArrayList<String> fetchValue(String userId,String orgId,String courseId) {     // Fetch the value for Library
		...
		try {
			...
			String sql="  select distinct(subject_id), b.subname from knowledgeassets_info a, subject_description b where  "
			+ "  a.uploaded_by = '"+userId+"' and a.status = 'Inactive' and  "
			+ "  a.course_id = '"+courseId+"' and  "
			+ "  b.subject_id = a.subject and b.organization_id = '"+orgId+"'";

			preparedStatement = con.prepareStatement(sql);

			resultSet = preparedStatement.executeQuery();
			System.out.println(" the query in FacilitatorManageSubjectDao="+sql);
			while(resultSet.next()) 
			{
				String subjectid=resultSet.getString(1);
				String subjectname=resultSet.getString(2);
				rDomain1.setSubjectId(subjectid);
				rDomain1.setSubjectName(subjectname);
...
public ArrayList<String> viewValue(String userId,String orgId,String courseId) {     // Fetch the value for Library
		...
		try {
			...
			+ "  b.subject_id = a.subject and b.organization_id = '"+orgId+"'";*/
			String sql="  select distinct(subject_id), b.subname from knowledgeassets_info a, subject_description b where  "
					+ "  a.uploaded_by = '"+userId+"' and a.course_id = '"+courseId+"' and  "
			+ "  b.subject_id = a.subject and b.organization_id = '"+orgId+"'";
			preparedStatement = con.prepareStatement(sql);
