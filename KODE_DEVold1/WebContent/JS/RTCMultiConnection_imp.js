...
// Added by Manish to show name in Reconnecting 18 19
var participants,reconnectuser;
var hashmapforuserid={};
var useridforusers;
var array = [];
...
(function() {
    ...
    window.RTCMultiConnection = function(channel) {
        ...
        function joinSession(session, joinAs) {
        	var splittedSession ="audio,video,data,screen";   // Participants who has joined the session dont want to share the Screen, So make Screen as false;        													
        	splittedSession = splittedSession.split(",");
        	 for (var i = 0; i < splittedSession.length; i++) {
             	//alert("Session Iteration "+splittedSession[i] +"  length= "+splittedSession.length +" i="+i);
        		 if(i==3){
        			 connection.session[splittedSession[i]] = false; // only Screen will be false
        		 }
        		 else
        			 connection.session[splittedSession[i]] = true; // rest all will be True
             }
            if (isString(session)) {
...
//By Manish
                    var remUser = _config.userid.split("-",1);
                   array.indexOf(remUser+" , ") === -1 ? array.push(remUser+" , ") : console.log("This item already exists");
                    customFunctionForUpdateList();
                    customFunctionForUserCount();
    ...
    function getRandomString() {
    	...
    	//Added by Manish to add name in Reconnecting 3223 and comment 3225 to 3234
    	return participants+'-'+new Date().getTime().toString(36).replace(/\./g, '');
...
// by manish
        if(enabled)
        {
        var remUser = e.root.userid.split("-",1);
        myDelete(remUser+" , ");
         customFunctionForUpdateList();
         customFunctionForUserCount();
        }
...
if(connection.numberOfConnectedUsers==1)
 array.indexOf(participants+" , ") === -1 ? array.push(participants+" , ") : console.log("This item already exists");
  customFunctionForUpdateList(); 
  customFunctionForUserCount();
  //till here
...
sdp = sdp.replace( /a=mid:audio\r\n/g , 'a=mid:audio\r\nb=AS:80\r\n');// previous 50  a=mid:audio\r\nb=AS:50\r\n
    ...
    function setDefaults(connection) {
        ...
        connection.bandwidth = {
            screen: 50 // 300kbps (dirty workaround)to decrease the bandwidth
        };
        ...
        connection.resources = {
            ...
            firebase: 'https://webrtc-experiment.firebaseIO.com/',
        ...
        connection.processSdp = function(sdp) {
            // process sdp here
        	//sdp = sdp.replace(/a=mid:audio\r\n/g, 'a=mid:audio\r\na=fmtp:111 x-google-min-bitrate=50; x-google-max-bitrate=128\r\n');
sdp = sdp.replace(/a=mid:audio\r\n/g, 'a=mid:audio\r\na=fmtp:111 x-google-min-bitrate=80; x-google-max-bitrate=90\r\n');
            return sdp;
        };
...
document.getElementById("exit-session").style.visibility = "visible";
        ...
        connection.onleave = function(e) {
            log('onleave', toStr(e));
connection.deletePeer(e.userid); // added by manish to remove stuck video
        };
        ...
        connection.onconnected = function(event) {
        	...
        	// Added by Manish to show name in Reconnecting 6598 to 6601
        	getdata();
            log('Peer connection has been established between '+participants+'(you) and', event.userid);
            var remoteuser=event.userid;
            var remUser = remoteuser.split("-",1);
 //by manish
            var parti=participants.split("-",1);
                       array.indexOf(participants+" , ") === -1 ? array.push(participants+" , ") : console.log("This item already exists"+participants);
                       array.indexOf(remUser+" , ") === -1 ? array.push(remUser+" , ") : console.log("This item already exists"+remUser);
                        customFunctionForUpdateList();
                       customFunctionForUserCount();
                       //till mere
        };
        ...
        connection.ondisconnected = function(event) {
        	// Added by Manish to show name in Reconnecting 6611 to 6617
       	 error('Peer connection seems has been disconnected between '+participants+' you and', event.userid);
            // alert('disconnected '+event.userid);
             var remoteuser=event.userid;
             var remUser = remoteuser.split("-",1);
  //By Manish
             myDelete(remUser+" , ");
         ...
         //  error('Peer connection seems has been disconnected between you and', event.userid);
           $("#a").show();
           $("#a").html("A Network Problem Occured. "+remUser[0]+" Reconnecting...");
             setTimeout(function(){ 
               $("#a").html(" ");
             }, 5000);
            connection.peers['user-id'].renegotiate();
            ...
            }, 10000); // 10 sec to remove stuck video
        };
...
//By Manish
            var remUser = event.streamid.split("-",1);
                       //array.push(remUser+" , ");
...
//Added by Manish to show name in Reconnecting 6761 to 6778
...
//implemented hash map by Manish
function getdata()
{
hashmapforuserid[useridforusers]=participants;
for(var i in hashmapforuserid) {
    if (hashmapforuserid.hasOwnProperty(i)) {
    	reconnectuser=hashmapforuserid[i];
    	        console.log('Key is: ' + i + '. Value is: ' + hashmapforuserid[i]);
//By Manish
                array.indexOf(hashmapforuserid[i]+" , ") === -1 ? array.push(hashmapforuserid[i]+" , ") : console.log("This item already exists");
                 customFunctionForUpdateList();
                 customFunctionForUserCount();
                console.log('nameee='+array);
               customUserName();
               //till here
    }
}
...
//Creating own method for deleting data from an array By Manish
...
function  customFunctionForUpdateList()
{
    $("#abc").html(array);
}

function customFunctionForUserCount() {
 $("#count").html("# of Users : "+array.length);                
}
