...
function getAudioElement(e, t) {
	...
	if (!e.nodeName || e.nodeName.toLowerCase() != "audio"
			&& e.nodeName.toLowerCase() != "video") {
		var n = e;
		e = document.createElement("audio");
		e[!!navigator.mozGetUserMedia ? "mozSrcObject" : "src"] = !!navigator.mozGetUserMedia ? n
				: window.webkitURL.createObjectURL(n)
	}
	t.toggle = t.toggle || [];
	...
	r.className = "media-container rrr";
	var i = document.createElement("div");
	i.className = "media-controls";
	r.appendChild(i);
	var s = document.createElement("div");
	s.className = "control "
			+ (t.toggle.has("mute-audio") ? "unmute-audio selected"
					: "mute-audio");
	i.appendChild(s);
	s.style["border-top-left-radius"] = "5px";
	s.onclick = function() {
		if (s.className.indexOf("unmute-audio") != -1) {
			s.className = s.className.replace("unmute-audio selected",
					"mute-audio");
			e.muted = false;
			if (t.onUnMuted)
				t.onUnMuted("audio")
		} else {
			s.className = s.className.replace("mute-audio",
	...
	if (!t.buttons || t.buttons && t.buttons.indexOf("record-audio") != -1) {
		...
		o.className = "control "
				+ (t.toggle.has("record-audio") ? "stop-recording-audio selected"
						: "record-audio");
		i.appendChild(o);
		o.onclick = function() {
			if (o.className.indexOf("stop-recording-audio") != -1) {
				o.className = o.className.replace(
						"stop-recording-audio selected", "record-audio");
				if (t.onRecordingStopped)
					t.onRecordingStopped("audio")
			} else {
				o.className = o.className.replace("record-audio",
...
function getMediaElement(e, t) {
	// d=volume input range  div;
	// l = volume controll div;
	// s =  media controlls div  width = 100%
	// i = media container  //
	//
	function E() {
		s.style.marginLeft = i.clientWidth - s.clientWidth - 5 + "px";
		if (d) {
			//d.style.width =  i.clientWidth / 3 + "px";
			d.style.width =   i.clientWidth - 110 + "px";
	...
	if (!e.nodeName || e.nodeName.toLowerCase() != "audio"
		...
		var n = e;
		e = document.createElement(n.getVideoTracks().length ? "video"
				: "audio");
		e[!!navigator.mozGetUserMedia ? "mozSrcObject" : "src"] = !!navigator.mozGetUserMedia ? n
				: window.webkitURL.createObjectURL(n)
	}
	...
	e.controls = false;
	var r = t.buttons
			|| [ "mute-audio", "mute-video", "full-screen", "volume-slider",
					"stop" ];
	r.has = function(e) {
	...
	var i = document.createElement("div");
	i.className = "media-container sss";
	//i.label = "Zakir";
	//alert(i.label);
	var s = document.createElement("div");
	...
	if (r.has("mute-audio")) {
		...
		o.onclick = function() {
			if (o.className.indexOf("unmute-audio") != -1) {
				o.className = o.className.replace("unmute-audio selected",
						"mute-audio");
				e.muted = false;
				e.volume = 1;
				if (t.onUnMuted)
				...
				o.className = o.className.replace("mute-audio",
						"unmute-audio selected");
				e.muted = true;
				e.volume = 0;
				if (t.onMuted)
	...
	if (r.has("mute-video")) {
		var u = document.createElement("div");
		u.className = "control "
				+ (t.toggle.has("mute-video") ? "unmute-video selected"
						: "mute-video");
		s.appendChild(u);
		u.onclick = function() {
			if (u.className.indexOf("unmute-video") != -1) {
				u.className = u.className.replace("unmute-video selected",
						"mute-video");
				e.muted = false;
				e.volume = 1;
				e.play();
				...
				u.className = u.className.replace("mute-video",
	...
	if (r.has("record-audio")) {
		...
		c.onclick = function() {
			if (c.className.indexOf("stop-recording-audio") != -1) {
				...
				c.className = c.className.replace("record-audio",
						"stop-recording-audio selected");
				if (t.onRecordingStarted)
					t.onRecordingStarted("audio")
			}
	...
	if (r.has("record-video")) {
		...
		h.className = "control "
				+ (t.toggle.has("record-video") ? "stop-recording-video selected"
						: "record-video");
		l.appendChild(h);
		h.onclick = function() {
			if (h.className.indexOf("stop-recording-video") != -1) {
				h.className = h.className.replace(
						"stop-recording-video selected", "record-video");
				if (t.onRecordingStopped)
					t.onRecordingStopped("video")
			} else {
				h.className = h.className.replace("record-video",
	...
	if (r.has("full-screen")) {
		...
		v.className = "control "
				+ (t.toggle.has("zoom-in") ? "zoom-out selected" : "zoom-in");
		if (!d && !c && !h && v) {
		...
		v.onclick = function() {
			if (v.className.indexOf("zoom-out") != -1) {
				v.className = v.className.replace("zoom-out selected",
						"zoom-in");
				g()
			} else {
				v.className = v.className.replace("zoom-in",
						"zoom-out selected");
				m(i)
			}
		};
		...
		function y(e) {
			if (e.srcElement != i)
				return;
			var n = document.webkitIsFullScreen || document.mozFullScreen
					|| document.fullscreen;
			/*i.style.width = (n ? window.innerWidth - 20 : t.width) + "px";*/
			
			i.style.width = (n ? window.innerWidth - 20 : 100 ) + "px"; 
			console.log("window.innerwith "+window.innerWidth - 20);
			console.log("t.width "+t.width);
			console.log("final width "+i.style.width);
			...
			if (t.height) {
				...
				b.style.height = (n ? 100 + "px"  : t.height + "px" )  ;
			}
			if (!n && t.onZoomout)
				t.onZoomout();
			if (n && t.onZoomin)
	...
	if (r.has("volume-slider") || r.has("full-screen") || r.has("record-audio")
			|| r.has("record-video")) {
		i.appendChild(l)
	}
	...
	//alert("new id="+e.fullName);
	b.className = "media-box";
	//b.innerHTML = '<h2>' + e.extra.fullName + '</h2>';//added by prity
	i.appendChild(b);
	b.appendChild(e);
	if (!t.width)
		t.width = innerWidth / 2 - 50;
	//i.style.width = t.width + "px";  //// media container-width first maximize
	i.style.width = 100+"px";
	i.style.height = 100+"px";
	if (t.height) {
		b.style.height = t.height + "px"; 
	}
	//alert("new box created id="+b.id);
	//b.querySelector("video").style.maxHeight = innerHeight + "px";  ---by prity
	var w = 0;
	if (t.showOnMouseEnter || typeof t.showOnMouseEnter === "undefined") {
