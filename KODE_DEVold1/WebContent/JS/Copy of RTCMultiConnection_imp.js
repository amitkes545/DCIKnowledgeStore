...
(function() {
    ...
    function RTCMultiSession(connection, callbackForSignalingReady) {
      ...
      /*  function getName1(name)
       ...
       // var user_id=document.body.getElementsByTagName('user_id');
        //alert(user_id);
        /*function ClassRoomOut()
        {
        	//alert("in function ClassRoomInOut");
        	var user_id = $('#rtcscript').attr("user_id");
        	var privilege = $('#rtcscript').attr("privilege");
        	var sesid = $('#rtcscript').attr("session_id");
        	alert(sesid);
        	//alert(privilege);
             
      
        	window.location.href="/KODE_DEV/ControllerServlet/ClassRoomRefreshOut?userid="+user_id+"&privilege="+privilege+"&session_id="+sesid;
