...
<script type="text/javascript" src="../JS/ParticipantSubjects.js"></script>
...
function institutions(){
	...
	document.Semesters.action="/KODE_DEV/ControllerServlet/ParticipantSubjects";
...
DepartmentsDomain d=arl.get(0);
session.setAttribute("DepartmentsName", d.getImageLocation());

Iterator<DepartmentsDomain> it= arl.iterator();
while(it.hasNext())
{
	DepartmentsDomain dd=it.next();
	 //System.out.println("web:"+dd.getDepartmentsNmae());
%>
	<li><a href="/KODE_DEV/ControllerServlet/ParticipantSubjects?SemesterName=<%=dd.getDepartmentsNmae()%>" class="sem_list"><%=dd.getDepartmentsNmae()%></a></li>
	<%-- <input type="submit" class="sem_list" name="SemesterName" value="<%=dd.getDepartmentsNmae()%>" onclick="institutions()" /> --%><br/>
<%  
 } %>
</ul>
</form>
...
<form name="SubjectDesc1" method="post" action="/KODE_DEV/ControllerServlet/ParticipantSubjectDescription">
...
ArrayList<DepartmentsDomain> arl1=(ArrayList<DepartmentsDomain>)request.getAttribute("depart");
	...
	while(it1.hasNext())
    {
		...
		for(String s:s1){
    %>
		<input type="submit" name="Subjects" value="<%=s%>"  /><br/>
  <%  
		}
  }
}
