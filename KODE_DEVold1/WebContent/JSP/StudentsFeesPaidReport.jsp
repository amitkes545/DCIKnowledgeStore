...
function actionGetList(){
	...
	if(courseval=="")
		...
		document.ksStore.action="/KODE_DEV/ControllerServlet/MISReportListService?RepID=MIS_FI_001&Course="+courseval+"&ExpRepFormat=None";
...
var x = document.getElementById("tableColumn").rows.length;
	...
	if(x>1)
		if(ExportVal=="PDF")
		{
			document.ksStore.action="/KODE_DEV/ControllerServlet/MISReportListService?RepID=MIS_FI_001&&Course="+courseval+"&ExpRepFormat="+ExportVal;
			document.ksStore.submit();		
...
<body>
	...
	<div class="container">
		...
		<div class="postpon_mod">
			<div style="text-align: center;">
				<p class="strong">Students Fees Payment Status</p>
	...
	<div class="table_section1">
		...
		<thead>
		 <tr class="row_head"  >
			...
			<th width="20%">Course Name <div class="header_stop" style="width:20%;">Course Name </div></th>
			<th width="20%">Fees Details<div class="header_stop" style="width:20%;">Fees Details</div></th>
			<th width="20%">Amount<div class="header_stop" style="width:20%;">Amount</div></th>
			<th width="20%">Status<div class="header_stop" style="width:20%;">Status</div></th>
		...
		<td width="20%">
		<%=stuinfo.get("CourseName").toString().trim()%>
		</td>
		<td width="20%">
		<%=stuinfo.get("mlt_fee_name").toString().trim()%>
		</td>
		<td width="20%">
		<%=stuinfo.get("fee_amount").toString().trim()%>
		</td>
		<td width="20%">
		<%=stuinfo.get("status").toString().trim()%>
		</td>
	...
	<select name="exportval" id="exportval" value=""class="opt" onchange="resetDownloadPath()">
