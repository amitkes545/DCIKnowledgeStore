...
<div class="certi" style="clear: both;">
...
<table>
	...
	ArrayList<DisplayCoursesDomain> arl=pcl.selectAssignCertifications(session.getAttribute("userId").toString());
	...
	if(dc.getCourseName().equals("null")){%>
		
		<td><%= %></td>
		
		
	<%} else { %>
	<td><%=dc.getCourseName() %></td>
	<%} %>
	<%
	
	if(dc.getImagePath().equals("null")){%>
		
		<td><%= %></td>
		
		
	<%} else { %>
	<td><%=dc.getImagePath() %></td>
	<%} %> --%>
	<%-- <% if(dc.getCourseName() != null)
{%>
<td><%= out.println("hi")%></td>
<% }%> --%>
	...
	<%}else{ %>
	<td>
	<a href="../JSP/GetAssignCertificate.jsp?id=<%=dc.getCourseName() %>&mark=<%=dc.getImagePath() %>&result=<%=dc.getCourseFee() %>&status=<%=dc.getCourseDetails() %>">Get Certificate </a></td>
<%} %>
	<%-- <div class="anch"><a href="../JSP/GetAssignCertificate.jsp?id=<%=dc.getCourseName() %>&mark=<%=dc.getImagePath() %>&result=<%=dc.getCourseFee() %>&status=<%=dc.getCourseDetails() %>">Get Certificate </a></div> --%>
<%
}
%>
</tr>
