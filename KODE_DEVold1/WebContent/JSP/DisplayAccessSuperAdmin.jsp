		...
		<div class="product_access" style="height: 280px;">
			...
			<% 
				 ...
				 if (request.getAttribute("OwnerSuccess")!= null) { 
					...
					%>
					<p style="color:green; font-size:14px; font-weight: bold; top: 18%; left: 26%; position: absolute;"><%= request.getAttribute("OwnerSuccess") %></p>				
					<%
				} 
				else  if (request.getAttribute("OwnerFailure")!= null) { 
					
					%>
					<p style="color:red; font-size:18px; font-weight: bold; top: 15%; left: 30%; position: absolute;"><%= request.getAttribute("OwnerFailure") %></p>					
					<%
				} 
				
			%>
			<form
				action="/KODE_DEV/ControllerServlet/DisplayAccessAdminServlet"
				method="post">
				<table border="2">
