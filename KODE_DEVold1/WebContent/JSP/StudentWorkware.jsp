...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@page import="com.kds.KODE_DEV.dao.GetCourseForStudentDao" %>
	    		...
	    		console.log("json response :",jsonResponse);
  	     		...
  	     		$.each(jsonResponse, function(index, value) {
		    	 		 ...
		    	 		 console.log('Actual obj value here :',value.SubjectId);
		    	 			// $('<option>').val(value.subjectId).text(value.subjectId).appendTo(select);
...
// alert("selectedValue="+selectedValue);
	    $.get('${pageContext.request.contextPath}/GAFStudent', {selectedValue : selectedValue},
	    		  function(jsonResponse) {
	    		console.log("json response :",jsonResponse);
   	     		...
   	     		jsonResponse = $.parseJSON(jsonResponse);
   	     		console.log("parsed json response :", jsonResponse);
   	     		$('<option>').val('').text('Select Assignments').appendTo(select);
   	     	$('<option>').val('').text('Select Assignments').appendTo(select);	
   	     	
   	     		$.each(jsonResponse, function(index, value) {
		    	 		  console.log('Index value here :',index);
		    	 		 console.log('Actual obj value here :',value.assignmenId);
		    	 			 $('<option>').val(value.assignmenId).text(value.assignmenId).appendTo(select);
		    	      }); 
...
if(username==null)
	response.sendRedirect("../JSP/error.jsp?errmsg=Session Expired");
        ...
        assessmentDomainForAssessment = (ArrayList<AssessmentsDetailsDomain>)request.getAttribute("assessmentDetails");
		//2nd form
...
function getDataForStudent(){
	alert('in');
	var assignid=document.getElementById("sessionID2").value;
	alert(assignid);
	
	//var uploadby=document.getElementById("uploadby").value;
	var uploadby="faculty@srm";
	document.cerify.action="/KODE_DEV/ControllerServlet/ParticipantAssesmentDetailsService?assessmentId="+assignid+"&facultyName="+uploadby+"";
	document.cerify.submit();
}
/* function clickAssessmentID(){
	//alert("in");
	var assessid=document.getElementById("sessionID2").value;
	alert(assessid);
	//var uploadby=document.getElementById("uploadby").value;
	var uploadby="faculty@srm";
	//alert("assessment id:"+assessid);
	//alert("uploadby id:"+uploadby);
	document.cerify.action="/KODE_DEV/ControllerServlet/ParticipantAssesmentDetailsService?assessmentId="+assessid+"&facultyName="+uploadby+"";
	document.cerify.submit();
}
 */	
...
String failureMsg = (String)request.getAttribute("failureMessage");
System.out.println("fff="+failureMsg);
 %>
...
<p class="success"><%=fileMessage %></p>
	<%} %>
		<p class="strong">Assignment</p>
		<%if(request.getAttribute("FacultySuccess")!=null){ 
			...
			//System.out.println("message value:"+request.getAttribute("successMessage"));%>
			  <p class="success">  <%= request.getAttribute("successMessage")%></p>
			     <%} 
			     System.out.println("fff="+request.getAttribute("failureMessage"));
			     if(request.getAttribute("failureMessage")!=null) { %>
			     <p class="failure"><%= request.getAttribute("failureMessage") %></p>
			     <%} 
		...
		<form name="cerify" id="cerify" method="post" action="/KODE_DEV/ControllerServlet/ParticipantAssignmentDetailsService" enctype="multipart/form-data">
							...
							<%-- 	 <% 
								 ...
								 ParticipantAssessmentsDao PAssessDao = new ParticipantAssessmentsDao();
                           	    ...
                           	    ArrayList<RetriveImagesDomain> arrayListForAssessment = new ArrayList<RetriveImagesDomain>();
                          	   	arrayListForAssessment = PAssessDao.selectCourses(ri);
	                ...
	                <select name="course" class="box_lng required" id="course" onchange="loadSubject()" style="margin-left:0px;">
		<option value="">Select Course*</option>
		 ...
		 <select name="subject" class="box_lng required" id="subject" onchange="loadAssignments()" style="margin-left:0px;">
							    	<option value="">Select Subject</option>
							    	
							    </select>
	
		<select name="sessionID2" placeholder="Session Name"  class="opt" id="sessionID2"style="margin-left:0px;">
		<option value="">Select Assignements</option>
		...
		<option value="All">All</option>
			...
			<input type="button" value="Find" id="find" onclick="getDataForStudent();"style="font-size:20px">
		</span>
		...
		<input class="box1" type="text" id="search" placeholder="Search"style="margin-right:0px;">
		</div> -->
...
<th width="12.5%">Sl #<div class="header_stop" style="width:12.5%;">Sl #</div></th>
		...
		<th width="12.5%">Upload<div class="header_stop" style="width:12.5%;">Upload</div></th>
		<th width="12.5%">Action<div class="header_stop" style="width:12.5%;">Action</div></th>
		</tr>
		<thead>
		 <% int count=1;
...
String ddmmyyyy_format = newFormat.format(date_display);
			...
			<td>
		   <input type ="text" name="assessmentId<%=assessmentDomainWithVlaue.getAssessmentId()%>" readonly="readonly" value="<%=assessmentDomainWithVlaue.getAssessmentId()%>" class="<%=assessmentDomainWithVlaue.getAssessmentId()%>"/>
			</td>
			<td align="left">
			...
			<td align="left"><a href="/KODE_DEV/ControllerServlet/AssignDownloadfile?filePath=/home/ftpuser1<%=assessmentDomainWithVlaue.getFilePath()%>"><%=s1[s1.length-1] %></a>
			</td>
			<td align="left">
			<%if(s2!=null){ %>
...
<%-- <select id="courses" class="select-box" style="margin-bottom:10px;">
	<option value="">Select Course*</option>
	<%
										String result="";
										
										GetCourseForStudentDao getCourseForStudent = new GetCourseForStudentDao();
										List<CourseStudentDomain> courses = getCourseForStudent.getCourseForStudent(facultyId);
										for(CourseStudentDomain course : courses)
										{
											System.out.println("course :"+course.getCourseId());
											result = course.getCourseId();
