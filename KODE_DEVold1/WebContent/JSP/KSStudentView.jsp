...
<%if (request.getAttribute("failureMessage")!= null) { 
	 String msg=(String)request.getAttribute("failureMessage");
...
<input type="button" id="but" onclick="clickKsId()" value="View" /><!-- style="display:none;" -->
	...
	<% if(assetsDomain1.getFilePath()!=null) {%>
		...
		<a href="/KODE_DEV/ControllerServlet/StudentClassRoomFileDownloadServlet?filepath=<%=assetsDomain1.getFilePath()%>&filename=<%=file[i]%>&action=<%=assetsDomain1.getAction()%>"><%=file[i]%></a>
		<%}else{
