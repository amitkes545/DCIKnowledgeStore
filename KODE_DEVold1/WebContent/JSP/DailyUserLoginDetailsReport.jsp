...
function actionGetList(){
	...
	document.ksStore.action="/KODE_DEV/ControllerServlet/MISReportListService?RepID=MIS_UA_001&startDate="+startDate+"&endDate="+endDate+"&Role="+SearchBy+"&SearchVal="+SearchVal+"&ExpRepFormat=None";
...
function actionRunExport(){
	...
	// alert("ExportVal == "+ExportVal)
	var x = document.getElementById("tableColumn").rows.length;
	
if(x>1)
	if(ExportVal=="PDF")
	{
		document.ksStore.action="/KODE_DEV/ControllerServlet/MISReportListService?RepID=MIS_UA_001&startDate="+startDate+"&endDate="+endDate+"&Role="+SearchBy+"&SearchVal="+SearchVal+"&ExpRepFormat="+ExportVal;
		document.ksStore.submit();
	}else if(ExportVal=="Excel")
	{
		...
		var div=document.getElementById("ExportDiv");
		div.innerHTML="<a id='ExportLink' href='../JSP/DownloadExportFile.jsp' > Click here to Download Excel </a>"; /* target='_blank' */
	...
	<div class="container">
		...
		<div class="postpon_mod">
			<div style="text-align: center;">
				<p class="strong">Daily User Login Details</p>
		 		<form name="ksStore" method="post" >
		 			<div class="combo_box_column row">
		 				<input class="box1" value=" " type="text" id="SDate" placeholder="Start Date*" >
		 				<input class="box1" value=" " type="text" id="EDate" placeholder="End Date*" >
		...
		<thead>
		 <tr class="row_head">
			<th width="16.6%">Date<div class="header_stop" style="width:16.6%;">Date</div></th>
			<th width="16.6%">Privilege <div class="header_stop" style="width:16.6%;">Privilege</div></th>
			<th width="16.6%">User ID	<div class="header_stop" style="width:16.6%;">User ID</div></th>
			<th width="16.6%">User Name <div class="header_stop" style="width:16.6%;">User Name</div></th>
			<th width="16.6%">Time <div class="header_stop" style="width:16.6%;">Time</div></th>
			<th width="16.6%">Log-in Status<div class="header_stop" style="width:16.6%;">Log-In Status</div></th>
		</tr>
		</thead>
		...
		<td width="16.6%">
		<%= stuinfo.get("Time").toString().trim() %>
		</td>
		<td width="16.6%">
		<%= stuinfo.get("LogStatus").toString().trim() %>
		</td>
		</tr>
		 <% }
		}
	...
	<input id="Export" type="button" value="Export" onclick="actionRunExport()" class="btn_class" />
		<div id="ExportDiv"> 
			<% if(!ReportPath.trim().equalsIgnoreCase("None"))
				{ %>
					<a href='../JSP/ShowFile.jsp?ReportPath=<%=ReportPath %>' > Click here to Download PDF</a> <!-- target="_blank" -->
				<%	}%>
	 	</div>
	</div>
