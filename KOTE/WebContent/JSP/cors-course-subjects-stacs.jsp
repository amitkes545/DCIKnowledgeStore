...
<title>CORS - Course Subjects (STACS)</title>
...
List<StacsSubjectsDomain> SubjectDetails=ExcelSheetUtil.getSubjectDetails(session);
...
<section class="middle-column">
  <div class="container">
    <div class="page-title">
      <h3>CORS - Course Subjects <span>(STACS)</span></h3>
    </div>
    <form class="cine-setup" action="/KOTE/ControllerServlet/StacsService?From=Subjects" method="post">
    <div class="dwrapper">
      <table id="fixed_hdr2">
        <thead>
          <tr>
            <th class="header-cell col12">Subject ID</th>
            <th class="header-cell col12">Subject Name</th>
          </tr>
        </thead>
        <tbody>
        <%
        if(SubjectDetails!=null && SubjectDetails.size()>0)
        {
        	...
        	{
        		StacsSubjectsDomain row=SubjectDetails.get(i);
        		
        		String subIdRowKey="SubID"+i;;
        		String subNameRowKey="SubName"+i;
        		
        		String SubID="";if(row.getSubjectId()!=null)SubID=row.getSubjectId();
        		String SubName=""; if(row.getSubjectName()!=null) SubName=row.getSubjectName();
        		%>
        		<tr class="light-bg">
                <td class="body-cell col12"><input type="text" name="<%=subIdRowKey %>" class="txtOnly" placeholder="Enter Subject ID" value="<%=SubID %>" /></td>
                <td class="body-cell col12"><input type="text" name="<%=subNameRowKey %>" class="txtOnly" placeholder="Enter Subject Name" value="<%=SubName %>" /></td>
              </tr>
...
<script type="text/javascript">
  ...
  $('.circle4').circleProgress({
    ...
    $(this).find('strong').html(Math.round(14 * progress) + '<span>% STACS</span>');
  });
</script>
<script>
