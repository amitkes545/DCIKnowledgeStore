...
<title>ANT - Administration Notification Template (STAMP)</title>
...
List<StampNMTDomain> NMTDetails=ExcelSheetUtil.getNMTDetails(session);
...
<form class="cine-setup" action="/KOTE/ControllerServlet/StampService?From=NMT" method="post">
  <div class="container">
    <div class="page-title">
      <h3>ANT - Administration Notification Template <span>(STAMP)</span></h3>
    </div>
    <div class="dwrapper">
      <table id="fixed_hdr2">
        <thead>
        ...
        if(NMTDetails!=null && NMTDetails.size()>0)
        {
        	for(int i=0;i<NMTDetails.size();i++)
        	{
				...
				String NotifyTypeval="";if(domain.getNotificationType()!=null)NotifyTypeval=domain.getNotificationType();
		...
		<tr class="light-bg"> <!-- <tr class="dark-bg"> -->
            <td class="body-cell col1 nomargpad"><div class="fillbox"><%=NotifyTypeval%> <input type="hidden"  class="txtOnly" id="<%= NotifyType%>"  name="<%= NotifyType%>" value="<%=NotifyTypeval %>" /></div></td>
...
<script type="text/javascript">
  ...
  $('.circle3').circleProgress({
    ...
    $(this).find('strong').html(Math.round(75 * progress) + '<span>% STAMP</span>');
  });
</script>
