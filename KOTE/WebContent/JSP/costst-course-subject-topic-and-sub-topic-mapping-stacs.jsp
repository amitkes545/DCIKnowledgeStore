...
List<StacsCOSTSTMapDomain> COSTSTMapDetails=ExcelSheetUtil.getCOSTSTMapDetails(session);
...
<section class="middle-column">
  <div class="container">
    <div class="page-title">
      <h3>CosTST - Course Subject, Topic and Sub Topic Mapping <span>(STACS)</span></h3>
    </div>
    <form class="cine-setup" action="/KOTE/ControllerServlet/StacsService?From=COSTSTMap" method="post">
    <div class="dwrapper">
      <table id="fixed_hdr2">
        <thead>
          <tr>
            ...
            <select name="<%=TopicIdRK %>" >
                ...
                <% if(TopicsList!=null && TopicsList.size()>0)
                {
                	for(int n=0;n<TopicsList.size();n++)
                	{
                		...
                		%>
                		<option value="<%=Topic_id_by_ts %>"  <% if(Topic_id_by_ts.trim().equalsIgnoreCase(TopicId)) { %> selected <% } %> ><%=Topic_id_by_ts %></option>
                <%	}
                }
                %>
            </select></td>
            ...
            <select name="<%=SubTopicIdRK %>" >
                ...
                <% if(SubTopicsList!=null && SubTopicsList.size()>0)
                {
                	for(int n=0;n<SubTopicsList.size();n++)
                	{
                		...
                		%>
                		<option value="<%=SubTopic_id_by_ts %>" <% if(SubTopic_id_by_ts.trim().equalsIgnoreCase(SubTopicId)) { %> selected <% } %> ><%=SubTopic_id_by_ts %></option>
                <%	}
                }
                %>
            </select>
...
<script type="text/javascript">
  ...
  $('.circle4').circleProgress({
    ...
    $(this).find('strong').html(Math.round(56 * progress) + '<span>% STACS</span>');
  });
</script>
<script>
