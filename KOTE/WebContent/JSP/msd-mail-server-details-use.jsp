...
<title>MSD - Mail Server Details (USE)</title>
...
List<UseMailServerDomain> paramDetails=ExcelSheetUtil.getUseMailServerDetails(session);
...
<section class="middle-column">
  <div class="container">
    <div class="page-title">
      <h3>MSD - Mail Server Details <span>(USE)</span></h3>
    </div>
    <form class="paramForm" id="paramForm" action="/KOTE/ControllerServlet/UseService?From=mailServer" method="post">
    <div class="dwrapper">
      <table id="fixed_hdr2">
        <thead>
          <tr>
            <th class="header-cell col13">Server Name</th>
            <th class="header-cell col13">SMTP Address</th>
            <th class="header-cell col13">Host</th>
            <th class="header-cell col13">SMTP Port Number</th>
            <th class="header-cell col1"><input type="checkbox" name="chkbox" id="select_all" /></th>
          </tr>
        </thead>
        <tbody>
        <%if(paramDetails!=null && paramDetails.size()>0){
        	for(int i=0;i<paramDetails.size();i++) {
        		UseMailServerDomain useParam = paramDetails.get(i);
        		
        		String name="";if(useParam.getName()!=null)name=useParam.getName();
        		String address="";if(useParam.getAddress()!=null)address=useParam.getAddress();
        		String host="";if(useParam.getHost()!=null)host=useParam.getHost();
        		String port="";if(useParam.getPort()!=null)port=useParam.getPort();
            ...
            <td class="body-cell col13"><input type="text" name="name<%=i%>" class="txtOnly"  placeholder="Enter Server Name" id="name<%=i%>" value="<%=name%>" maxlength="100" /></td>
            <td class="body-cell col13"><input type="text" name="address<%=i%>" class="txtOnly"  placeholder="Enter SMTP Address" id="address<%=i%>" value="<%=address%>"  maxlength="25" /></td>
            <td class="body-cell col13"><input type="text" name="host<%=i%>" class="textnumber" placeholder="Enter Host" id="host<%=i%>" class="number-only" maxlength="25" value="<%=host%>"/></td>
            <td class="body-cell col13"><input type="text" name="port<%=i%>"class="number-only"  placeholder="Enter SMTP Port Number" id="port<%=i%>" class="number-only" maxlength="5" value="<%=port%>"/></td>
            <td class="body-cell col1"><input type="checkbox" name="check1" class="children_checkbox" /></td>
        	<% }%>
        <%}%>
...
<script type="text/javascript">
  ...
  $('.circle2').circleProgress({
    ...
    $(this).find('strong').html(Math.floor(14 * progress) + '<span>% USE</span>');
  });
</script>
<script>
