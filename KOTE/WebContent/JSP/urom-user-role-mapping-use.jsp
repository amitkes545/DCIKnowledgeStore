...
<title>URoM - User Role Mapping (USE)</title>
...
List<UseRoleMapDomain> paramDetails=ExcelSheetUtil.getUseRoleMap(session);
...
<section class="middle-column">
  <div class="container">
    <div class="page-title">
      <h3>URoM - User Role Mapping <span>(USE)</span></h3>
    </div>
    <form class="paramForm" id="paramForm" action="/KOTE/ControllerServlet/UseService?From=roleMap" method="post">
    <div class="dwrapper">
      <table id="fixed_hdr2">
        <thead>
          <tr>
            <th class="header-cell col12">User ID</th>
            <th class="header-cell col12">Role ID</th>
          </tr>
        </thead>
        <tbody>
         <%if(paramDetails!=null && paramDetails.size()>0){
        	for(int i=0;i<paramDetails.size();i++) {
        		UseRoleMapDomain useParam = paramDetails.get(i);

        		String userId="";if(useParam.getUserId()!=null)userId=useParam.getUserId();
        		String roleId="";if(useParam.getRoleId()!=null)roleId=useParam.getRoleId();
            	...
            	if(AllUsersList!=null && AllUsersList.size()>0)
            	{
            		for(int k=0;k<AllUsersList.size();k++)
            		{
            			...
            			if(UidMapping.containsKey(id))
            			{
            				...
            				if(uid_ts.trim().equalsIgnoreCase(userId))
            					selected="selected";
            ...
            <select name="roleId<%=i%>">
            	...
            	if(RoleList!=null && RoleList.size()>0)
            	{
            		...
            		%>	<option value="<%=id %>" <% if(id.trim().equalsIgnoreCase(roleId)) { %> selected <%} %> ><%=id %></option> <%
            		}
            	}
            	%>
            </select></td>
...
<script type="text/javascript">
  ...
  $('.circle2').circleProgress({
    ...
    $(this).find('strong').html(Math.floor(56 * progress) + '<span>% USE</span>');
  });
</script>
<script>
